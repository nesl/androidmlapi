...
/**
 * A class for writing XDR files. Not too hard to do in Java since the XDR
...
public class XDROutputStream extends DataOutputStream implements XDRDataOutput {
    ...
    public void writeDoubleArray(double[] array, int start, int n)
            throws IOException {
        writeInt(n);
        for (int i = start; i < n; i++)
            writeDouble(array[i]);
    ...
    public void writeFloatArray(float[] array, int start, int n)
            throws IOException {
        writeInt(n);
        for (int i = start; i < n; i++)
            writeFloat(array[i]);
